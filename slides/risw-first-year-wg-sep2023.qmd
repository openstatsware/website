...
- The [R Validation Hub](https://www.pharmar.org/) is an important working group here
...
- We have run comparison analyses with other R packages, namely `nlme`, `glmmTMB` and `lme4` as well as SAS `PROC GLIMMIX` and have found that `mmrm` is the fastest of them and provides closes results to `PROC GLIMMIX`
...
- Workshop "Good Software Engineering Practice for R Packages" on world tour
  - Basel, Shanghai, San José, Rockville MD (last Tuesday), Montreal (16 and 17 Oct) [🌎](https://openpharma.github.io/workshop-r-swe-mtl/)
